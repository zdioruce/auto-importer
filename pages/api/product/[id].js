...
export default async function handler(req, res) {
  ...
  try {
    const result = await excuteQuery({
      query: 'INSERT INTO products (title, tags, brand, description, images, status) VALUES (?, ?, ?, ?, ?, ?)',
      values: [productData.title, tags.join(), productData.brand, productData.description, productData.imagesCSV, 0],
    });
